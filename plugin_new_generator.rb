...
task default: :test
  ...
  module Generators
    ...
    protected
      ...
      def camelized
        @camelized ||= name.split('/').map{|n| n.gsub(/\W/, '_').squeeze('_').camelize}.join('::')
      end

      def valid_const?
        if original_name =~ /[^0-9a-zA-Z\/_]+/
          raise Error, "Invalid plugin name #{original_name}. Please give a name which use only alphabetic or numeric or \"_\" or / characters."
        elsif camelized =~ /^\d/
